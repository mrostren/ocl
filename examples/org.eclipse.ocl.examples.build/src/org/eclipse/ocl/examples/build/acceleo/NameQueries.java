...
//			prevCharIsLower = charIsLowerCase;
	...
	public List<org.eclipse.ocl.examples.pivot.Class> getAllSuperClasses(org.eclipse.ocl.examples.pivot.Class aClass) {
		...
		return new ArrayList<org.eclipse.ocl.examples.pivot.Class>(results.keySet());
	}
	
	public int getAllSuperClasses(Map<org.eclipse.ocl.examples.pivot.Class, Integer> results, org.eclipse.ocl.examples.pivot.Class aClass) {
	...
	public int getDepth(org.eclipse.ocl.examples.pivot.Class aClass) {
		Map<org.eclipse.ocl.examples.pivot.Class, Integer> results = new HashMap<org.eclipse.ocl.examples.pivot.Class, Integer>();
	...
	public org.eclipse.ocl.examples.pivot.Package getPivotPackage(EPackage ePackage) {
		...
		TypeManagerResourceSetAdapter resourceSetAdapter = TypeManagerResourceSetAdapter.getAdapter(resourceSet, null);
		Ecore2Pivot ecore2Pivot = Ecore2Pivot.getAdapter(resource, resourceSetAdapter.getTypeManager());
		...
		if (pivotPackage.getNsURI().equals(PivotPackage.eNS_URI)) {
			mergeLibrary(resourceSetAdapter.getTypeManager(), pivotPackage);
		}
...
//		}
	...
	public ExpressionInOcl getExpressionInOcl(NamedElement contextElement, OpaqueExpression specification) {
		...
		ResourceSet resourceSet = resource.getResourceSet();
		TypeManager typeManager = TypeManager.getAdapter(resourceSet);
		String expression = PivotUtil.getBody(specification);
		URI uri = typeManager.getResourceIdentifier(contextElement, null);
		try {
			return PivotUtil.resolveSpecification(typeManager, uri, contextElement, expression);
	...
	public void mergeLibrary(TypeManager typeManager, org.eclipse.ocl.examples.pivot.Package primaryPackage) {
//		primaryPackage.setName("ocl");
		...
		for (Library library : typeManager.getLibraries()) {
			...
			for (Type secondaryType : libraryTypes) {
				...
				if (primaryType != null) {
					org.eclipse.ocl.examples.pivot.Class primaryClass = (org.eclipse.ocl.examples.pivot.Class)primaryType;
					List<org.eclipse.ocl.examples.pivot.Class> primarySuperClasses = primaryClass.getSuperClasses();
					org.eclipse.ocl.examples.pivot.Class secondaryClass = (org.eclipse.ocl.examples.pivot.Class)secondaryType;
					for (org.eclipse.ocl.examples.pivot.Class secondarySuperClass : secondaryClass.getSuperClasses()) {
						org.eclipse.ocl.examples.pivot.Class primarySuperClass = (org.eclipse.ocl.examples.pivot.Class)typeMap.get(secondarySuperClass);
					...
					primaryClass.getOwnedOperations().addAll(secondaryClass.getOwnedOperations());
					primaryClass.getOwnedAttributes().addAll(secondaryClass.getOwnedAttributes());
				}
			}
		}
		for (Type primaryType : primaryTypes) {
			if (primaryType instanceof org.eclipse.ocl.examples.pivot.Class) {
				List<org.eclipse.ocl.examples.pivot.Class> primarySuperClasses = ((org.eclipse.ocl.examples.pivot.Class)primaryType).getSuperClasses();
