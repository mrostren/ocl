...
class FrameStackRegion {
	 ...
	 */
	public void leave(DomainExpression entryPoint, boolean disposeFrameWithSpecifiedEntryPoint) {
		if (!isEmpty()) {
	...
	protected boolean isLoopAst(DomainExpression ast) {
		return ast != null && (ASTUtil.getType(ast) == ASTUtil.FOR || ASTUtil.getType(ast) == ASTUtil.WHILE);
	}

	private void disposeFramesUntil(DomainExpression entryPoint) {
