 ...
 ******************************************************************************/
package org.eclipse.ocl.examples.debug.util;
...
public class EclipseUtil {
	...
	public static IFile findIFile(DomainExpression ast) {
		if (ast == null) {
			return null;
		}
		...
		if (ASTUtil.getFile(ast) != null) {
			final Path path = new Path(ASTUtil.getFile(ast).getAbsolutePath());
			return workspaceRoot.getFileForLocation(path);
		}
		if (ASTUtil.getUri(ast) != null) {
			try {
				final URI fileURI = FileLocator.toFileURL(ASTUtil.getUri(ast).toURL()).toURI();
				final IFile[] files = workspaceRoot.findFilesForLocationURI(fileURI);
				if (files.length > 0) {
					return files[0];
				}
			} catch (Exception ex) {
				OCLDebugPlugin.getDefault().logException(ex);
			}
		}
	...
	public static void openEditorAt(DomainExpression astNode) {
		final IFile file = findIFile(astNode);
		if (file != null) {
			openEditorAt(file, ASTUtil.getLine(astNode), ASTUtil.getColumn(astNode), true);
		}
	}
