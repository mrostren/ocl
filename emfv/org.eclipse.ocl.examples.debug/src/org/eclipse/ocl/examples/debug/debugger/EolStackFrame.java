...
import org.eclipse.ocl.examples.debug.execute.context.Frame;
...
public class EolStackFrame extends EolDebugElement implements IStackFrame {
	...
	public IVariable[] getVariables() throws DebugException {
		...
		for (Variable v : frame.getAll().values()) {
			eolVariables[i] = new EolVariable(getDebugTarget(), v.getName(), ASTUtil.getValue(v));
			i++;
		}
	...
	public int getLineNumber() throws DebugException {
		if (frame.getCurrentStatement() != null) {
			return ASTUtil.getLine(frame.getCurrentStatement());
		}
		return -1;
	}
	...
	private int getCharStart(final DomainExpression ast) throws DebugException {
		if (ast != null) {
			final IDocument doc = getDocument(ast);
			try {
				if (doc != null) {
						return doc.getLineOffset(ASTUtil.getLine(ast) - 1);
				}
	...
	private int getCharEnd(final DomainExpression ast) throws DebugException {
		if (ast != null) {
			final IDocument doc = getDocument(ast);
			try {
				if (doc != null) {
					return doc.getLineOffset(ASTUtil.getLine(ast));
				}
			} catch (BadLocationException e) {
				OCLDebugPlugin.getDefault().logException(e);
			}
	...
	private IDocument getDocument(final DomainExpression current) {
